...
function importer(options) {
  return resolveSource(options).pipe(rxjs.mergeMap(({
    ...
    const context = {
      ...options,
      workingPath,
      ndjson: () => aspectsNdjsonPath ? fs$1.createReadStream(aspectsNdjsonPath) : null
    };
...
function resolveAspectData({
  ...
  return rxjs.mergeMap((resolvedAsset) => {
    const ndjsonStream = ndjson();
    return ndjsonStream ? rxjs.from(findNdjsonEntry(ndjsonStream, (line) => typeof line == "object" && line !== null && "filename" in line && line.filename === resolvedAsset.originalFilename)).pipe(rxjs.map((aspectsFromImport) => ({
      ...resolvedAsset,
