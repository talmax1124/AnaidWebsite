"use strict";
var path = require("node:path"), generateHelpUrl_esm = require("./generate-help-url.esm.js"), resolveFrom = require("resolve-from"), semver = require("semver"), timing = require("./timing.js"), execa = require("execa"), oneline = require("oneline"), _package = require("./package.js"), _internal = require("./_internal.js"), preferredPM = require("preferred-pm"), which = require("which");
...
async function checkStudioDependencyVersions(workDir) {
  const manifest = await timing.readPackageJson(path__default.default.join(workDir, "package.json")), dependencies = {
    ...
    if (!dependency)
      return !1;
    const manifestPath = resolveFrom__default.default.silent(workDir, path__default.default.join(pkg.name, "package.json")), installed = semver__default.default.coerce(manifestPath ? (await timing.readPackageManifest(manifestPath)).version : dependency.replace(/[\D.]/g, ""));
    if (!installed)
      return !1;
...
async function checkRequiredDependencies(context) {
  ...
  const {
    workDir: studioPath,
    output
  } = context, [studioPackageManifest, installedStyledComponentsVersion, installedSanityVersion] = await Promise.all([await timing.readPackageManifest(path__default.default.join(studioPath, "package.json"), defaultStudioManifestProps), await timing.readModuleVersion(studioPath, "styled-components"), await timing.readModuleVersion(studioPath, "sanity")]), wantedStyledComponentsVersionRange = _package.peerDependencies["styled-components"];
