...
const CONNECTION_TIMEOUT$1 = 15 * 1e3, READ_TIMEOUT$1 = 180 * 1e3, request$1 = getIt.getIt([middleware.keepAlive(), middleware.promise()]);
...
const CONNECTION_TIMEOUT = 15 * 1e3, READ_TIMEOUT = 180 * 1e3, request = getIt.getIt([middleware.keepAlive(), middleware.promise()]);
...
const MINIMUM_API_VERSION = "2025-02-19", importDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    if (isUrl)
      ...
      sourceIsFolder = fileStats.isDirectory(), sourceIsFolder ? inputStream = sourceFile : (assetsBase = path__default.default.dirname(sourceFile), inputStream = fs.createReadStream(sourceFile));
    }
...
`, listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    ...
    output.print(`Opening ${manageUrl}`), await open__default.default(manageUrl);
  }
}, deleteHookCommand = {
  name: "delete",
  group: "hook",
